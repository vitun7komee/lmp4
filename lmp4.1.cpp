...
void sum(int start, int end, int& result) {
    ...
    result = res;
    std::cout << "Thread " << std::this_thread::get_id() << " sum: " << result << std::endl; // Вывод результата работы потока
}
...
int main() {
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        threads[i] = std::thread(sum, i * (ARRAY_SIZE / THREAD_COUNT), (i + 1) * (ARRAY_SIZE / THREAD_COUNT), std::ref(results[i]));
    }
    ...
    // Сумма всех результатов потоков
    int total = 0;
    for (int i = 0; i < THREAD_COUNT; i++) {
        total += results[i];
    }
    ...
    std::cout << "Total sum: " << total << std::endl; // Вывод результата
